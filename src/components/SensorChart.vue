<template>
  <div class="sensor-chart">
    ...
    <div class="room-selector">
      ...
      <select id="room-select" v-model="selectedRoomId" @change="onRoomChange">
...
// Используем глобальное состояние выбранной комнаты из хранилища
// const selectedRoomId = computed({
//   get: () => houseStore.selectedRoomId,
//   set: (value: string) => {
//     houseStore.selectRoom(value);
//   }
// });

// Обработчик изменения выбранной комнаты в селекте
const onRoomChange = (event: Event) => {
  const select = event.target as HTMLSelectElement;
  houseStore.selectRoom(select.value);
};
...
onMounted(() => {
// Select first room by default if no room is selected
